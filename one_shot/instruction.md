# Instruction — one_shot (`samseg_long_onestep`)
...
- Worker script: `one_stage_samseg_long.py`, called as
...
```bash
BUNDLE=/ABS/PATH/TO/nipoppy_samseg_long/one_shot   # this folder
...
- `BUNDLE` must be this directory (contains `config.json`, `one_stage_samseg_long.py`, …).
...
(`requirements.txt` lives at the repository root, shared with `two_shot`.)
...
`python <DATASET>/pipelines/processing/samseg_long_onestep-1.0.0/one_stage_samseg_long.py <bids> <output> sub-<PARTICIPANT>`
and `--no-container`.
...
' > "$DATASET/one_stage_${PARTICIPANT}.log" 2>&1 < /dev/null &
echo "started; log: $DATASET/one_stage_${PARTICIPANT}.log"
```

Watch progress: `tail -f "$DATASET/one_stage_${PARTICIPANT}.log"`.
